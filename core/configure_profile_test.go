...
func TestConfigController_GetAllServiceKeys(t *testing.T) {
	...
	if len(allServiceKeys) != 99 {
		t.Errorf("the total number of all service key: expect 99, actual %d\n", len(allServiceKeys))
	}
	for i := 0;i < 99;i++ {
		serviceKey := fmt.Sprintf("test.servicekey.key-%d\n", i + 1)
...
func TestConfigController_RemoveService(t *testing.T) {
	//1：删除成功
	configController.RemoveService("test.servicekey.key-20", 20)
